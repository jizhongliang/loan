 ...
 */
@Component
public class CreditBorrowPaySuccessConsumer extends OnsConsumerBase {

    private static final Logger logger = LoggerFactory.getLogger(CreditBorrowPaySuccessConsumer.class);
    @Value("${ons.borrow.consumer.paySuccessId}")
    private String id;
    @Value("${ons.borrow.consumer.paySuccessTag}")
    private String tag;
    @Value("${ons.topic}")
    ...
    protected boolean doConsume(HwcOnsContext context) {
        ...
        if (context.getData() instanceof JSONObject) {
            ...
            borrowGenXinNotifyService.paySuccessNotify(jsonObject);
        }
        return false;
    }
}
