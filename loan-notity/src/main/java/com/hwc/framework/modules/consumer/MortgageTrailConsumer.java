 ...
 */
@Component
public class MortgageTrailConsumer extends OnsConsumerBase {

    private static final Logger logger = LoggerFactory.getLogger(MortgageTrailConsumer.class);
    @Value("${ons.borrow.consumer.mortgageTrailId}")
    private String id;
    @Value("${ons.borrow.consumer.mortgageTrailTag}")
    private String tag;
    @Value("${ons.topic}")
    ...
    protected boolean doConsume(HwcOnsContext context) {
        ...
        if (context.getData() instanceof JSONObject) {
            ...
            mortgageGenXinNotifyService.mortgageTrailNotify(jsonObject);
        }
        return false;
    }
}
