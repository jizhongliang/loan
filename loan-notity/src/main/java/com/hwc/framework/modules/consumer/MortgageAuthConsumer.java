 ...
 */
@Component
public class MortgageAuthConsumer extends OnsConsumerBase {

    private static final Logger logger = LoggerFactory.getLogger(MortgageAuthConsumer.class);
    @Value("${ons.borrow.consumer.mortgageAuthId}")
    private String id;
    @Value("${ons.borrow.consumer.mortgageAuthTag}")
    private String tag;
    @Value("${ons.topic}")
    ...
    protected boolean doConsume(HwcOnsContext context) {
        ...
        if (context.getData() instanceof JSONObject) {
            ...
            mortgageGenXinNotifyService.mortgageAuthNotify(jsonObject);
        }
        return false;
    }
}
