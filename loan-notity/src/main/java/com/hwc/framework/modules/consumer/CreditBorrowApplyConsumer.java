 ...
 */
@Component
public class CreditBorrowApplyConsumer extends OnsConsumerBase {

    private static final Logger logger = LoggerFactory.getLogger(CreditBorrowApplyConsumer.class);
    @Value("${ons.borrow.consumer.applyBorrowId}")
    private String id;
    @Value("${ons.borrow.consumer.applyBorrowTag}")
    private String tag;
    @Value("${ons.topic}")
    ...
    protected boolean doConsume(HwcOnsContext context) {
        ...
        if (context.getData() instanceof JSONObject) {
            ...
            borrowGenXinNotifyService.borrowApply(jsonObject);
        }
        return false;
    }
}
