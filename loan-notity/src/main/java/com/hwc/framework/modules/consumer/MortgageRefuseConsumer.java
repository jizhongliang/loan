 ...
 */
@Component
public class MortgageRefuseConsumer extends OnsConsumerBase {

    private static final Logger logger = LoggerFactory.getLogger(MortgageRefuseConsumer.class);
    @Value("${ons.borrow.consumer.mortgageRefuseId}")
    private String id;
    @Value("${ons.borrow.consumer.mortgageRefuseTag}")
    private String tag;
    @Value("${ons.topic}")
    ...
    protected boolean doConsume(HwcOnsContext context) {
        ...
        if (context.getData() instanceof JSONObject) {
            ...
            mortgageGenXinNotifyService.mortgageRefuseNotify(jsonObject);
        }
        return false;
    }
}
