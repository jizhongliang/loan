 ...
 */
@Component
public class MortgageReviewConsumer extends OnsConsumerBase {

    private static final Logger logger = LoggerFactory.getLogger(MortgageReviewConsumer.class);
    @Value("${ons.borrow.consumer.mortgageReviewId}")
    private String id;
    @Value("${ons.borrow.consumer.mortgageReviewTag}")
    private String tag;
    @Value("${ons.topic}")
    ...
    protected boolean doConsume(HwcOnsContext context) {
        ...
        if (context.getData() instanceof JSONObject) {
            ...
            mortgageGenXinNotifyService.mortgageReviewNotify(jsonObject);
        }
        return false;
    }
}
