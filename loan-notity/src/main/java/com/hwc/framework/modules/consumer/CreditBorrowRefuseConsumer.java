 ...
 */
@Component
public class CreditBorrowRefuseConsumer extends OnsConsumerBase {

    private static final Logger logger = LoggerFactory.getLogger(CreditBorrowRefuseConsumer.class);
    @Value("${ons.borrow.consumer.refuseBorrowId}")
    private String id;
    @Value("${ons.borrow.consumer.refuseBorrowTag}")
    private String tag;
    @Value("${ons.topic}")
    ...
    protected boolean doConsume(HwcOnsContext context) {
        ...
        if (context.getData() instanceof JSONObject) {
            ...
            borrowGenXinNotifyService.refuseBorrowNotify(jsonObject);
        }
        return false;
    }
}
