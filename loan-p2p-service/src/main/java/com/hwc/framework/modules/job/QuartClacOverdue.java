...
@Component
@Lazy(value = false)
public class QuartClacOverdue implements Job {

	private static final Logger logger = LoggerFactory.getLogger(QuartClacOverdue.class);

	
	public String clacOverdue() {
		logger.info("计算逾期--开始...");
		BorrowRepayService borrowRepayService = SpringContextHolder.getBean(BorrowRepayService.class);
		ClQuartzInfo info = borrowRepayService.clacOverdue();
		logger.info("计算逾期--完成...");
	...
	public void execute(JobExecutionContext arg0) throws JobExecutionException {
		...
		// 查询当前任务信息
		ClQuartzInfo quartzInfo = quartzInfoService.findByCode("Overdue");
